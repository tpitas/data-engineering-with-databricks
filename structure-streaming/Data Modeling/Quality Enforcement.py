...
# MAGIC
# MAGIC -- Fails due to constraints
...
# MAGIC
# MAGIC -- Query returned no results
...
# MAGIC
# MAGIC -- < > rows in spark_catalog.bookstore.orders_silver violate the new CHECK constraint (quantity > 0)
...
query = (spark.readStream.table("bronze")
        ...
        .filter("quantity > 0")
     .writeStream
        .option("checkpointLocation", "dbfs:/mnt/demo_bookstore/checkpoints/orders_silver")                                  
...
#dbutils.fs.rm("dbfs:/mnt/demo_bookstore/checkpoints/orders_silver", True)  tb